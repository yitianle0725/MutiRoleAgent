...
角色灵魂加载器
==============
从 ``prompts/soul/*.md`` 加载角色灵魂文件，支持缓存、热重载和元数据提取。

相比 V2 composer.py 的 _load_soul()，增强点：
...
class LoadedSoul:
    ...
    def get_core_prompt(self) -> str:
        """获取核心扮演提示词（身份 + 性格 + 说话方式 + 规则）。

        Returns:
            组装好的核心提示词文本。
        """
        parts: list[str] = []
        for section_name in ["身份", "性格", "说话方式", "扮演规则"]:
...
class SoulLoader:
    ...
    def load(self, name: str) -> LoadedSoul | None:
        ...
        if not filename:
            # Fallback: 从 composer 映射查找
            from prompts.composer import _PERSONA_TO_SOUL
            filename = _PERSONA_TO_SOUL.get(name, "")

        if not filename:
            logger.warning(f"[SoulLoader] 未知角色: {name}")
            return None

        filepath = self._soul_dir / filename
